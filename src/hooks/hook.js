...
const useCheckBox = (initial) => {
  ...
  function renderItems() {
    return checkbox.map(({ title, id, isCheck }, i) => {
      return (
        <div key={i} style={{ border: isCheck ? 'none' : '2px solid #ffffff' }}>
          <label
            className={
              isCheck
                ? 'request__form-label request__form-label_active'
                : 'request__form-label'
            }
            htmlFor={id}
...
const useParallax = ({ className, sensitivity }) => {
  const sens = sensitivity ? sensitivity : 300
  const [propss, sets] = useSpring(() => ({ xys: [0, 0, sens] }))

  const calc = (x, y, sens) => {
    return [
      -(y - window.innerHeight / 2) / sens,
      (x - window.innerWidth / 2) / sens,
      sens
    ]
  }
  const trans = (x, y) =>
    `perspective(600px) rotateX(${x}deg) rotateY(${y}deg) translateZ(${x}px)`
